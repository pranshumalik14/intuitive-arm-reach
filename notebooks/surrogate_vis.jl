...
using Surrogates, DataFrames, Flux, Statistics, LinearAlgebra, Plots, PlutoUI, Distributions, LaTeXStrings, CSV;
...
# ╔═╡ 53785491-ed57-456d-acd3-3f6bfafd88c2
md"
# Surrogacy Time!
"

# ╔═╡ ca6ee797-34bd-42a3-9efb-4d26330138da
D = 128;

# ╔═╡ ad90b910-85f4-470b-8fde-af26c1032254
md"
...
# ╔═╡ 071752e8-c897-40b4-85bf-32c135209180
begin
...
# ╔═╡ 745ad836-b609-46da-a54b-8e3e9bb48d1c
...
begin
	...
	B = 5; w = T/2B;              # num basis funcs, B, and width of basis funcs, w
...
# ╔═╡ a8e80f09-9400-4bb5-a0e6-29be4fec16fc
begin
	df_thetas   = CSV.read("../training_data/flattened_theta.csv", DataFrame);
	df_jointpos = CSV.read("../training_data/flattened_input.csv", DataFrame);
	pos_joints = reshape(df_jointpos."0"[N+2+1:end], (N+2, D-1)) |> transpose;
	thetas     = reshape(df_thetas."0"[N*B+1:end], (N*B, D-1)) |> transpose;
end;

# ╔═╡ cca4ea0c-f796-42e0-9e9c-f71fc1e3cdcc
ub = [maximum(pos_joints[:, i]) for i in 1:size(pos_joints, 2)]

# ╔═╡ 38eba73c-30db-4669-8c81-6ce9c38b4180
lb = [minimum(pos_joints[:, i]) for i in 1:size(pos_joints, 2)]

# ╔═╡ ea5f142d-8af5-40ce-a7f8-6e315ebefbe6
begin
	x_train = pos_joints |> matrixtotuple
	y_train = thetas |> matrixtotuple
	model = Chain(rbf(N+2, 100), Dense(100, 200, relu), Dense(200, N*B))
	...
	test_error = mean([norm(nn_surrogate(x_train[i]) - thetas[i, :])^2 for i in 1:D-1])
end
...
[[Surrogates]]
deps = ["Distributions", "ExtendableSparse", "GLM", "IterativeSolvers", "LatinHypercubeSampling", "LinearAlgebra", "Requires", "Sobol", "SparseArrays", "XGBoost", "Zygote"]
git-tree-sha1 = "7bf8afc2643866d705754631f249f0723a3871ca"
...
[[XGBoost_jll]]
deps = ["Artifacts", "CompilerSupportLibraries_jll", "JLLWrappers", "Libdl", "Pkg"]
git-tree-sha1 = "af5a07debf4747d9fe906d74180d29015700a317"
